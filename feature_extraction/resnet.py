...
## From https://github.com/azshue/AdvBN/blob/master/models/resnet.py
...
class Head(nn.Module):
    ...
    def forward_to_feature(self, x, tag='clean'):
        for i in range(self.layer_num):
            for j in range(self.block_num[i]):
                m = getattr(self, 'layer{}_{}'.format(str(int(self.cut) + i + 1), str(j)))
                x = m(x, tag)
...
class ResNet(nn.Module):
    ...
    def _forward_impl(self, x, stage, tag='clean'):
        if stage == 'feature_x':
            ...
            x = self.head(x, tag)
        elif stage == 'ffeature_x':
            x = self.feature_x(x)
            x = self.head.forward_to_feature(x)
        else:
            x = self.feature_x(x)
            x = self.head(x, tag)
        return x
