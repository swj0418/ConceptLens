...
export default class BiTree extends Component {
    ...
    setupImages = (selection) => {
        ...
        selection.each(function (d, i) {
                    ...
                    if (codeSample[v] && directionSample[h]) {
                        ...
                        if (v === 0 && methodDrawnCount < horizontalCount * topGcount) {
                            ...
                            selection
                                ...
                                // .attr('transform', `translate(${xPos + horizontalScale.bandwidth() / 2}, ${yPos - (imageSize / imageSizeAdjuster) - 2})`)
                                // .attr('transform', `translate(${xPos + horizontalScale.bandwidth() - (imageSize / imageSizeAdjuster)}, ${yPos - (imageSize / imageSizeAdjuster) - 2})`)
                                .attr('transform', `translate(${xPos + horizontalScale.bandwidth()}, ${yPos - (imageSize / imageSizeAdjuster) - 24})`)
                                .attr('fill', d => {
                                    let color = d3.hcl(
                                        methodColorScale.hue(methodName),
                                        methodColorScale.chr(methodName),
            ...
            if (okayToDrawDGlyph(glyphDirectionDrawn)) {
                for (var h = 0; h < horizontalCount; h++) {
                    const xPos = horizontalScale(h)
                    const yPos = verticalScale(0)
                    selection
                        .append('g')
            ...
            if (okayToDrawCGlyph(glyphCodeDrawn)) {
                for (var v = 0; v < verticalCount; v++) {
                    const xPos = horizontalScale(0)
                    const yPos = verticalScale(v)

                    selection
                        .append('g')
                        .attr('transform', `translate(${xPos - 20}, ${yPos})`)
                        .call(insertDirectionGlyph, v, verticalScale.bandwidth() / 1.2)
                        .call(insertDirectionGlyph, v, verticalScale.bandwidth() / 1.2)
                }
                glyphCodeDrawn++
            }
