...
from feature_extraction.feature_extraction import CLIPTransform
...
def infer_dataset_size(filenames, old=False):
    ...
    code_ids = sorted(list(set([fn.split("-")[0] for fn in filenames])))
    directions = [fn for fn in filenames if fn.split("-")[0] == code_ids[0]]
