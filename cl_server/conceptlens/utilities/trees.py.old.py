...
def compute_direction_coherence(tree,
    ...
    """
    Recurrence happens as an inner function, because this will prevent unnecessary copy of two feature matrices, which
    ...
    """
    assert var_mode in ['editdist', 'cosine', 'raw', 'new']
    ...
    def _compute_coherence_tree(node):
        ...
        if var_mode in ['new', 'raw']:
            mag_subset, var_subset = mag_features[:, leaf_indices], var_features[:, leaf_indices]
        else:
            mag_subset, var_subset = mag_features[:, leaf_indices], var_features[:, leaf_indices][:, :, leaf_indices]

        if var_mode not in ['new', 'raw']:
            new_var_subset = []
            for i, sub in enumerate(var_subset):
                new_var_subset.append(_get_off_diagonal_elements(sub))
            var_subset = torch.stack(new_var_subset)
        ...
        # var mode - [using edit dist, using cosine distance]
        var = 0
        if var_mode == 'editdist':
            var = torch.mean(torch.std(torch.norm(var_subset, dim=-1), dim=1), dtype=torch.bfloat16).item()
            if node['name'] == 135:
                print(f"EDIT DIST ({node['name']}): ", var)
        elif var_mode == 'cosine':
            # var = torch.mean(torch.std(torch.std(var_subset, dim=-1), dim=-1)).item()
            # var = torch.mean(torch.std(var_subset, dim=(1, 2))).item()  # This is wrong
            if var_subset.shape[2] == 1:
                var = torch.mean(var_subset).item()
            else:
                var = torch.mean(torch.std(torch.std(var_subset, dim=1), dim=-1)).item()
            # print(var, var_subset.shape)
            # if var_subset.shape[2] == 0:
            #     print(var_subset)
            #
            # if node['name'] == 135 or node['name'] == 122:
            #     print(f"COSINE DIST ({node['name']}): ", var)
            #     print("Leaf indices: ", leaf_indices)
            #     print("Var subset shape: ", var_subset.shape)
            #     print(var_subset[0])  # Just one code.
        elif var_mode == 'raw':
            # std across dimensions
            x = torch.std(var_subset, dim=-2, keepdim=True)  # [code, 1, dimension]
            # print("std: ", x.shape, " original: ", var_subset.shape)
            x = torch.mean(x, dim=-1, keepdim=True)
            var = torch.mean(x).item()
        elif var_mode == 'new':
            mean = torch.mean(var_subset, dim=1, keepdim=True)
            distances = var_subset - mean
            distances = torch.norm(distances, dim=-1, keepdim=True)
            std = torch.std(distances, dim=1, keepdim=True)
            var = torch.mean(std).item()
...
def compute_coherence_code(tree, mag_features, var_features, mag_mode, var_mode, collect_only_leaf,
                           code_indices=None,
                           direction_indices=None
                           ):
    """
    For code hierarchy. Code hierarchy differs from direciton hierarchy coherence computation in that
    you need to take subset of coherence features by code.
    ...
    will take most of the time.
     ...
     """
    assert var_mode in ['editdist', 'cosine']
    all_magnitude, all_variance = [], []
    ...
    def _compute_coherence_tree(node):
        ...
        magnitude = torch.mean(torch.mean(torch.norm(mag_subset, dim=-1), dim=0), dim=0).item()

        # var mode - [using edit dist, using cosine distance]
        var = 0
        if var_mode == 'editdist':
            var = torch.mean(torch.std(torch.norm(var_subset, dim=-1), dim=1)).item()
        elif var_mode == 'cosine':
            var = torch.mean(torch.std(torch.std(var_subset, dim=-1), dim=-1)).item()
